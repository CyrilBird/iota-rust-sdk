...
impl Input {
    ...
    pub fn object_id(&self) -> Option<&ObjectId> {
        match self {
            Self::Pure { .. } => None,
            Self::ImmutableOrOwned(obj_ref) | Self::Receiving(obj_ref) => {
                Some(&obj_ref.object_id)
            }
    ...
    /// If this is a `Shared` input, returns `(object_id, initial_shared_version, mutable)`.
    ...
    /// Returns the [`ObjectReference`] if this is an `ImmutableOrOwned` or `Receiving` input.
    ...
    /// Returns the [`ObjectReference`]s for objects being received in this input.
    ///
